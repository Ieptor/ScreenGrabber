...
pub fn main(){  
    ...
    if let Some((modifier, key1)) = shortcut_command {
        if let Some((modifier2, key2)) = shortcut_fs_command {
            ...
            if cfg!(target_os = "windows"){
                handle_windows_code(id1, id2, overlay_path, edit_path, shortcut_string, shortcut_fs);
            } else {
                handle_linux_code(id1, id2, id3, overlay_path, edit_path);
            }
        }
    }
}
...
fn handle_windows_code(id1: u32, id2: u32, overlay_path: String, edit_path: String, shortcut_string: String, shortcut_fs: String){
...
fn handle_linux_code(id1: u32, id2: u32, id3: u32, overlay_path: String, edit_path: String){
    loop {
        // Check for global hotkey events
        if let Ok(event) = GlobalHotKeyEvent::receiver().try_recv() {
            if event.id == id1 && event.state == HotKeyState::Released {
